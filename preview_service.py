...
def generate_brush_masks_sync(brush_name, stroke_w, stroke_h, tip_size, scale_coef=0.4, 
    ...
    try:
        ...
        # 1. СТАРАЯ НАДЕЖНАЯ ЛОГИКА: Нормализация размера кисти
        ...
        # Исправленные параметры рисования
        stroke_layer.paintPath(path, "Normal", "Normal") 
        
        taper_len = int(draw_w * 0.05)
        taper_start_end = QPointF(padding + taper_len, (draw_h / 2) - 1) 
        stroke_layer.paintLine(start_point.toPoint(), taper_start_end.toPoint(), 0.5, 1.0)
        
        taper_end_start = QPointF(draw_w - padding - taper_len, (draw_h / 2) - 1)
        stroke_layer.paintLine(taper_end_start.toPoint(), end_point.toPoint(), 1.0, 0.5)
        ...
        # Диагностика
        has_pixels = not all(b == 0 for b in stroke_bytes)
        print(f"[Brush Studio] Render check: Stroke has pixels? {has_pixels}")
